...
from sspm.data import CIFData
from sspm.data import collate_pool
    ...
    for i, (input, target, batch_cif_ids) in enumerate(val_loader):
        ...
        if model_args.task == 'regression':
            ...
            if test:
                ...
                test_target = target
                assert test_pred.shape[1] == 95
                # test_preds += test_pred[:, 1].tolist()
                # t = target.view(-1, 1).type(torch.int64)
                # tt = torch.gather(test_pred, 1, t)[:,0].tolist()
                # test_preds += tt
